...
function bindEvents(){
  ...
  $("#notesGrid").onclick=handleCardAction; $("#notesGrid").onchange=saveAlbumDate; $("#notesGrid").oncontextmenu=handleAlbumContextMenu; $("#closeReader").onclick=()=>$("#readerDialog").close(); $("#readerFav").onclick=()=>currentNote&&toggleFavorite(currentNote.id,true); $("#editNote").onclick=openEditor;
...
function handleAlbumContextMenu(e){const card=e.target.closest(".album-photo");if(!card)return;if(cloudMode&&!CloudApp.isAdmin)return;e.preventDefault();deleteNote(card.dataset.id)}
